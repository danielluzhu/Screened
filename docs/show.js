...
function render(show, data) {
  ...
  if (show.photo) {
    ...
    thumbSrc(img, `/Screened/shows/${show.photo}`, { priority: true });
    img.alt = show.name;
    head.append(img);
  }
  ...
  if (cast.length) {
    ...
    for (const character of cast) {
      ...
      if (character.photo) {
        ...
        thumbSrc(img, `/Screened/characters/${character.photo}`);
        img.alt = "";
        img.loading = "lazy";
        li.append(artLink(img, `/Screened/character/${character.slug}`));
      }
...
async function init() {
  const slug = decodeURIComponent(location.pathname.replace(/^\/Screened\/show\/?/, "").replace(/\/$/, "")).trim();
  let data;
  try {
