...
function render(film, data) {
  ...
  select.addEventListener("change", async () => {
    ...
    try {
      await post("/Screened/api/rating", { title: film.title, year: film.year, tier: select.value });
  ...
  if (related.length) {
    ...
    for (const series of related) {
      for (const other of series.films) {
        ...
        link.href = `/Screened/film/${other.slug}`;
...
function factsList(film) {
  ...
  if (film.year) {
    ...
    link.href = `/Screened/year/${film.year}`;
    dd.append(link);
    dl.append(dd);
  }
...
function detailsForm(film, data, done) {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      const result = await post("/Screened/api/film/details", payload);
      toast("Details saved");
      done(result);
    } catch (err) {
      status.textContent = "";
...
function editor({ cls, heading, rows, value, placeholder, field, film }) {
  ...
  const commit = async () => {
    ...
    status.textContent = "Saving…";
    try {
      await post("/Screened/api/film/critique", { title: film.title, year: film.year, [field]: next });
      saved = next;
      film[field] = next;
