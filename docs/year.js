...
function watchedRow(film) {
  ...
  if (film.poster) {
    ...
    thumbSrc(img, `/Screened/posters/${film.poster}`);
    img.alt = "";
    img.loading = "lazy";
...
function render(entry, data) {
  ...
  if (i > 0) {
    ...
    prev.href = `/Screened/year/${years[i - 1]}`;
    nav.append(prev);
  }
  if (i >= 0 && i < years.length - 1) {
    const next = text("a", null, `${years[i + 1]} →`);
    next.href = `/Screened/year/${years[i + 1]}`;
    nav.append(next);
  }
  main.append(nav);
...
async function init() {
  const raw = decodeURIComponent(location.pathname.replace(/^\/Screened\/year\/?/, "").replace(/\/$/, "")).trim();
