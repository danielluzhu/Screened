...
// Same write the director page uses: nothing is added until this is clicked.
async function addFilm(film, button) {
  ...
  try {
    ...
    toast(`${film.title} added, unrated`);
  } catch (err) {
...
function render(film, data) {
  ...
  const add = text("button", "add-other", "+ Add to my list");
  add.type = "button";
  add.addEventListener("click", () => addFilm(film, add));
  note.append(add);
  main.append(note);
  ...
  for (const who of film.directors ?? []) {
    ...
    back.href = `/director/${director.slug}`;
    box.append(back);
    main.append(box);
  }
}
...
async function init() {
  ...
  const slug = decodeURIComponent(location.pathname.replace(/^\/other\/?/, "")).trim();
  ...
  let data, otherFilms;
  try {
    const [a, b] = await Promise.all([fetch("/api/data"), fetch("/api/other-films.json")]);
