...
function render(film, data) {
  ...
  if (film.poster) {
    ...
    thumbSrc(img, `/posters/${film.poster}`, { priority: true });
...
async function loadDetail(slug) {
  try {
    const res = await fetch(`/api/detail/${slug}.json`);
    if (!res.ok) return {};
...
async function init() {
  ...
  try {
    ...
    const [res, loaded] = await Promise.all([fetch("/api/data"), loadDetail(slug)]);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    data = await res.json();
